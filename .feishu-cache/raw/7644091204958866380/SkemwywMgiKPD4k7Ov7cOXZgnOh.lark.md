...
> **目标读者**：在生产环境中部署 Claude Code 的团队。**独立学习者**：请参见入门指南。
...
for FILE in $STAGED; do
    if [[ "<equation>FILE" =~ \.(ts|tsx|js|jsx|py)</equation> ]]; then
        # 检查核心逻辑中的 TODO（非测试）
...
if [[ "$TOOL" == "Bash" ]]; then
    ...
    if [[ "<equation>COMMAND" =~ ^(npm|pnpm|yarn)[[:space:]]+install</equation> ]]; then
        exit 0
    fi
fi
...
if [[ "$TOOL" == "Write" ]] || [[ "$TOOL" == "Edit" ]]; then
    ...
    # 检查模式违规
    if [[ "<equation>FILE" =~ \.(tsx?)</equation> ]]; then
        CONTENT=$(cat "$FILE")
...
**来源**：Alan 工程团队（Charles Gorintin、Maxime Le Bras），2026 年 2 月
...
此悖论的智能体侧对应是 「TDD 工作流」中记录的验证缺口模式。
...
- 《Claude Code：从入门到大师》 §9.12 Git 最佳实践 — 提交工作流、计划→执行模式
- 「安全加固指南」 — MCP 安全、密钥保护、钩子堆栈
- 「数据隐私指南」 — MCP 数据库风险、保留策略
- 「企业 AI 治理」 — 组织级治理：使用章程、MCP 审批流程、护栏级别、合规
- 「采用方式」 — 团队设置、共享规范、企业发布
- 计划模式 — 执行前的安全探索
- 权限系统 — 允许/拒绝规则、钩子
...
while true; do
  ...
  if [[ -f "$HEARTBEAT_FILE" ]]; then
    ...
    if [[ "$AGE" -gt "$TIMEOUT" ]]; then
      echo "看门狗：<equation>{AGE}秒无心跳（限制：</equation>{TIMEOUT}秒）——正在终止进程组"
...
**版本**：1.0.0**最后更新**：2026-01-21**变更日志**：基于社区验证的生产模式的初始版本