...
if command -v jq &>/dev/null && command -v yq &>/dev/null; then
  ...
  for mcp in $ACTIVE; do
    if ! echo "<equation>APPROVED" | grep -q "^</equation>{mcp}$"; then
...
# 复制级别配置
curl -s "<equation>CONFIG_REPO/raw/main/templates/.claude/settings.</equation>{TIER}.json" \
  -o .claude/settings.json

# 复制 CLAUDE.md 模板
curl -s "<equation>CONFIG_REPO/raw/main/templates/CLAUDE.md.</equation>{TIER}" \
  -o CLAUDE.md
...
LOG_FILE="<equation>LOG_DIR/</equation>(date +%Y-%m-%d).jsonl"
...
> 来源：飞书 · AI Spark AI Wiki ｜ 原文（最新版）：<https://lcnniolukk80.feishu.cn/wiki/Z0X3wMmGhiPbMBkF0sOcjzHRnQg> ｜ 归档：2026-06-22
