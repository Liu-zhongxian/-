...
> 版本 3.41.0 · 最后更新：2026 年 5 月 · 来源：claude-code-ultimate-guide
...
**小贴士**：这些都不是"秘密"——都在 [Claude Code CHANGELOG](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md) 里。去读吧！
...
**增强状态栏（ccstatusline）：** 添加到 `~/.claude/settings.json`：
...
**⚠️ 限制**：TaskList 仅显示 `id`、`subject`、`status`、`blockedBy`。查看 `description`/`metadata` → 对每个任务使用 `TaskGet(taskId)`。
...
5. **每个项目都创建 CLAUDE.md**
6. **每完成一个任务就提交**
7. **了解发送了什么** — 提示词、文件、MCP 结果 → Anthropic（可在 claude.ai/settings/data-privacy-controls 退出训练数据使用）
...
- **白皮书（中英文）**：通过 PDF 生成工作流导出（共 10 份专题）
- **项目记忆**：在项目根目录创建 `CLAUDE.md`
- **DeepSeek（经济实惠）**：通过 `ANTHROPIC_BASE_URL` 配置

---

*来源：claude-code-ultimate-guide v3.41.0 · Florian BRUNIAUX · 最后更新：2026 年 5 月*