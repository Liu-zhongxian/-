...
await writeFile(
  `prompts/debug/<equation>{evaluatorName}-</equation>{timestamp}.md`,
  fullPrompt,
  "utf-8",
);
// 现在才调用提供商
...
function canUseUnifiedMode(context: MultiFileContext, maxTokens = DEFAULT_MAX_UNIFIED_TOKENS) {
  if (context.totalTokenEstimate > maxTokens) {
    return {
      viable: false,
      reason: `合并内容（约 <equation>{context.totalTokenEstimate} Token（词元））超出限制（</equation>{maxTokens}）`,
    };
  }
  return { viable: true };
}
...
> 来源：飞书 · AI Spark 知识库 ｜ 原文（最新版）：<https://lcnniolukk80.feishu.cn/wiki/F5Lbwgr8siCF5hk9dfbcwlUzn2W> ｜ 归档：2026-05-29
