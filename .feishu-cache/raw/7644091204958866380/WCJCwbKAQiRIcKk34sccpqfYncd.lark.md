...
2. **八个核心工具**：Bash 工具（万能适配器）、读取工具、编辑工具、写入工具、搜索工具、文件匹配工具、任务工具（子智能体）、TodoWrite。这就是全部工具集。**搜索策略演变**：早期 Claude Code 版本曾尝试使用 Voyage 嵌入实现语义代码搜索的 RAG。Anthropic 内部基准测试显示，基于 ripgrep 的 agentic 搜索表现更优、运维复杂度更低——无需索引同步，也不存在外部嵌入提供商的安全风险——因此切换至此方案。这一"搜索而非索引"的哲学以简单性和安全性换取了延迟和 Token 成本。社区插件（用于 AST 模式的 ast-grep）和 MCP 服务器（用于符号的 Serena、用于 RAG（检索增强生成）的 grepai）可满足专业化需求。*来源*：[Latent Space 播客](https://www.latent.space/p/claude-code)（2025 年 5 月）、ast-grep 文档
...
<whiteboard token="QF73wSSGShEeX1btJS8cQLbxnqf"></whiteboard>

*参考 Mohamed Ali Ben Salem 的架构图——更深入的分解见「架构内部图表」。*
...
**置信度**：100%（Tier 1 - 官方）**来源**：[Anthropic 工程博客](https://www.anthropic.com/engineering/claude-code-best-practices)
...
- [ ] **技能作用域 Hooks** — 特定于技能执行上下文的事件钩子
  - 按技能管理生命周期
  - 参见：《Claude Code：从入门到大师》第 5.11 节
...
- [ ] **智能体团队** — 多智能体并行协调（实验性，v2.1.32+）
  - 基于 Git 的协调，自主任务认领
  - 参见：《Claude Code：从入门到大师》第 9.20 节
...
**来源**：综合自 Gur Sannikov 分析

---

## 2. 工具集

**置信度**：100%（Tier 1 - 官方）**来源**：[code.claude.com/docs](https://code.claude.com/docs/en/setup)
...
> **📖 深度阅读**：完整的多工具组合工作流，参见「搜索工具精通」。

---

## 3. 上下文管理内部机制

**置信度**：80%（Tier 2 - 部分官方）**来源**：
...
**置信度**：100%（Tier 1 - 官方 Anthropic 文档）**来源**：
...
**用户控制**：使用 `/compact` 在逻辑断点处手动触发摘要，或使用**会话交接**（参见会话交接）在压缩历史上保留意图。
...
**置信度**：100%（Tier 1 - 已记录行为）**来源**：[code.claude.com/docs](https://code.claude.com/docs/en/setup) + 系统提示（工具定义中可见）
...
**置信度**：100%（Tier 1 - 官方）**来源**：[code.claude.com/docs/en/sandboxing](https://code.claude.com/docs/en/sandboxing)
...
⚠️ **域名前置**：CDN（Cloudflare、Akamai）可绕过域名过滤⚠️ **Unix 套接字**：错误配置的 `allowUnixSockets` 会带来权限提升⚠️ **文件系统**：过于宽泛的写权限会使 `$PATH` 目录面临攻击
...
**深度阅读**：完整技术参考、配置示例和故障排除，参见「原生沙箱指南」。
...
→ **交叉参考**：完整示例见主指南第 7 节 - Hooks。

---

## 6. MCP 集成

**置信度**：100%（Tier 1 - 官方）**来源**：[code.claude.com/docs/en/mcp](https://code.claude.com/docs/en/mcp)
...
*图 1：MCP 架构，展示思考层（LLM）、控制层（MCP 服务器）和执行层（工具）之间的分离。设计参考 Dinesh Kumar 的 LinkedIn 可视化，在 Apache-2.0 许可证下重新创建。*
...
→ **交叉参考**：安全考量见第 8.6 节 - MCP 安全。

### MCP 扩展：Apps（SEP-1865）

**状态**：稳定（2026 年 1 月 26 日）**规范**：[GitHub 上的 SEP-1865](https://github.com/modelcontextprotocol/ext-apps)**联合作者**：OpenAI、Anthropic、MCP-UI 创建者
...
→ **交叉参考**：更广泛的 MCP 安全考量见第 8.6 节 - MCP 安全。
...
**置信度**：100%（Tier 1 - 官方）**来源**：[anthropic.com/engineering/advanced-tool-use](https://www.anthropic.com/engineering/advanced-tool-use)
...
**置信度**：90%（Tier 1 - 官方 Anthropic 工程）**来源**：[Anthropic 工程：高级工具调用](https://www.anthropic.com/engineering/advanced-tool-use) | [程序化工具调用文档](https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling)
...
**置信度**：100%（Tier 1 - 官方）**来源**：Daniela Amodei（Anthropic 联合创始人兼总裁）- 公开声明
...
**示例**：Gur Sannikov（嵌入式工程）采用了仅使用 Claude Code 原生能力（Hooks（钩子）、Skills（技能模块）、任务工具）的 ADR 驱动工作流，不依赖外部框架——验证了本指南中记录的架构哲学。
...
**置信度**：80%（Tier 2 - 基于 2026 年 3 月经过事实核查的数据）**来源**：官方文档、Perplexity 研究（2026 年 3 月）、各厂商变更日志
...
**最后更新**：2026 年 2 月**Claude Code 版本**：v2.1.34**文档版本**：1.1.0
