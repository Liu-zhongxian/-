...
![图片展示了飞书CLI配置应用时的界面。界面中显示“正在配置应用...”等信息，下方有一个二维码，提示使用飞书/ Lark 扫码配置应用。下方还有一行文字，说明可打开以下链接完成配置，并给出了链接地址。该图片与上文提到的配置应用会跳到网页，让你建一个飞书CLI应用的内容相关，直观呈现了配置应用时的扫码操作界面。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=MmJhNmRlN2E3NzY4MzE5MWUxYTdlMzIwYWRkNzQ1MDFfNzg0MjIyZWM3ODhlNzA2NTU5NDI0ZDYzMWMyYjE1MmRfSUQ6NzY0NDk3MzI4ODc4NzIwMTIyMF8xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)

配置应用会跳到网页，让你建一个飞书 CLI 应用。我建的应用名叫"小墨同学的飞书 CLI"，头像随便挑一个，点创建。

![图片展示的是飞书CLI应用创建的创建界面。界面上方显示“创建飞书 CLI 应用”，下方有头像选择区域，提供了多个卡通头像选项。名称输入框内已填写“小墨同学的飞书 CLI”。底部有一个蓝色的“创建”按钮。该图片与上文提到的在飞书应用市场创建飞书CLI应用的内容相关，直观呈现了创建应用时的界面和部分操作步骤。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=Y2JlYmNhM2E4MTI2ZDI4MjlkNTU0YzM4OTFhMjRhNjNfMTQwOTJkNTJmOWVlMzE1YWFhYmIwZTIwMjhjZmJhM2ZfSUQ6NzY0NDk3MzMyNzc2OTA3ODk4M18xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
...
![图片展示了飞书CLI应用配置成功后的界面。上方显示“OK: 应用配置成功！App ID: cli_aa8d5c454f785cc3”，并给出appId、appSecret（打码）、brand等信息。下方有“应用已配置”提示，以及“是否允许AI访问你个人的消息、文档、日历等飞书/Lark数据，并以你的名义执行操作？”的询问，用户选择了“全部权限”。该图片与上文提到的在终端应用配置飞书CLI，打印App ID及询问权限相关，直观呈现了配置结果及权限设置情况。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NzBlZjFmZTBkMmY5YTU0NmEzZDc4ODhmNDMwNGM2YWRfODBiM2IxZDM4MzI4MzY3MzA1NjY4MjNkYjQyZmI0NTJfSUQ6NzY0NDk3MzM1NDYzMzc5MjcwM18xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
...
![图片展示的是飞书CLI界面中“确定开通并授权以下权限吗？”的弹窗。弹窗上方显示“小墨同学的飞书 CLI”，下方列出多项权限，如添加、回复云文档中的评论，删除云文档中的评论等。弹窗底部有一个勾选框，提示“一并开通审批、多维表格、日历、通讯录、文档、消息与群组、邮箱、电子表格、幻灯片、任务、视频会议的常用权限”，并有“开通并授权”蓝色按钮。该图片与上文 addCriterion答案](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YzU0NDBjZmFiZDJmYWMzNDI4NzViMTE1YWE2YmYwMzZfNzRiZmZlNTBiNDQ3YjQzYjc2ZTUxZTdiNmU1MWQ2NGNfSUQ6NzY0NDk3MzM5MzE1NDAwMjEyN18xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
...
![图片展示的是飞书CLI创建测试文件的验证结果。上方显示“已处理34s”，并告知已创建并上传到飞书个人云空间根目录，文件名为“lark-cli-test-20260516.txt”，并附有链接。下方列出“1个文件已更改”，文件为“lark-cli-test-20260516.txt”，显示“+2 -0”。图片与上文提到的让Codex在自己文件夹创建测试文件并验证其在飞书个人云空间根目录存在的情况相呼应，展示了说明操作成功。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=MmViYTVjNzE1MWIxNmFlNmQzZDQzYzkxOGYzNTc4OTNfMGQ4NzUzMjg4OTllZmVlM2Y2ZDY2N2Q1YTFjMWQ3OGRfSUQ6NzY0NDk3MzQyMzE4NTM0OTgyNl8xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)

去飞书云盘里打开，文件确实在，内容也写进去了。能从命令行一路把文件创建并推到飞书，那飞书的CLI还是真的能完成工作。

![图片展示了飞书云盘中名为“lark-cli-test-20260516.txt”的文件内容。文件路径显示在AI Spark的小墨同学目录下，最近修改时间为刚刚。文件内容为“Lark CLI test file Created by Codex on 2026-05-16.”。这与上下文提到的去飞书云盘打开文件，且文件内容已写入相呼应，证明飞书的CLI能够从命令行创建文件并推送到飞书云盘，验证了其工作能力。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YmFiNTQyMDllOGJhMDRmNDk3MjRkMTRjNWQwMzU0OWJfNjgzNDdkNDJiNWIyMmE4YjFiMzg5NjFmZTk1ODZhOGNfSUQ6NzY0NDk3MzQ1NTg0NjQ3NzAxNV8xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
...
![图片展示的是飞书智能工作台中“月度战略大屏”界面。上方显示累计月营收24,866，历史最高月营收10,126。下方有最大来源占比趋势图、月总营收趋势柱状图、最大来源分布饼状图等图表，分别呈现营收趋势、来源占比及分布情况。该界面与上下文紧密相关，是对文档中提到的“飞书神器配合AI Agent，工作流彻底起飞”中“月度战略”部分的直观呈现，展示了飞书智能工作台在数据统计方面的功能。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZDlkMTc3NjVlMjc4ZmJlNThjOTY5ZTNkZWE3Mjc0NmRfMjNjZGZjMGRlNDQyNTQ0NmM4M2ZhZWE1YThkNTg4M2FfSUQ6NzY0NDk3MzQ5MDIxMDExNDc2MV8xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
...
![图片展示的是飞书CLI授权失败的提示信息。内容显示本次新授予的scope有contact:user.basic_profile:readonly和im:message:recall，还提示Scopes未授予的原因多样，如scope被禁用等，可通过lark 自动生成](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZjQ3NjhjMzkwM2MyYTgyMjlmNjBlNzVjODgyMjE5NTFfMDUzYzU0ODkzYzlkNjE1MzM4NDZhZjU3M2IzNjI5NWJfSUQ6NzY0NDk3MzUyMDcxOTYyOTUxNV8xNzgyMTA3MjUwOjE3ODIxMTA4NTBfVjM)
