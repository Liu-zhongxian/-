...
**作者**：Florian BRUNIAUX | 贡献者：Claude（Anthropic）**阅读时间**：约 25 分钟**最后更新**：2026 年 5 月
...
# 缺少错误处理
ast-grep -p 'async function $F($<equation>) { </equation>$BODY }' \
...
**背景**：在 Excalidraw（15.5 万行 TypeScript）上进行基准测试**作者**：YoanDev（grepai 维护者 — 存在潜在偏差）**方法**：5 个相同的代码发现问题
...
# 3. 模式检测（ast-grep）
# 识别所使用的生命周期方法
ast-grep 'componentDidMount() { $<equation>BODY }'
ast-grep 'componentWillReceiveProps(</equation>$) { $$$BODY }'
# → 映射到等效的 Hooks
...
# 2. 模式检测（ast-grep）
# 查找 N+1 查询模式
ast-grep 'for (<equation><equation>) { await db.query(</equation></equation>) }'
...
**最后更新**：2026 年 5 月**兼容版本**：Claude Code 2.1.7+